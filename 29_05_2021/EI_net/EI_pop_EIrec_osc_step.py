...
osc_f = 40
osc_A = 0.35
osc_trange = (Tsim/2, Tsim)
if osc_A > 0:
    for n in range(Ne):
        id = get_cell_id('pop_E', n)
        hvec[m] = add_osc_input(cells[id], 'soma', Tsim, dt, osc_f, osc_A, osc_trange)
        m += 1
...
# Firing rate vectors (full, before sthe pulse, during the pulse)
rvec = dict(); tvec = dict();
rvec['e'], tvec['e'] = calc_rvec('pop_E', bin_sz, t_range=(0, Tsim))
rvec['i'], tvec['i'] = calc_rvec('pop_I', bin_sz, t_range=(0, Tsim))

# Smooth firing rates
rvec_sm = dict(); tvec_sm = dict();
sm_len = 25
rvec_sm['e'], tvec_sm['e'] = smooth_data(rvec['e'], tvec['e'], win_len=sm_len) 
rvec_sm['i'], tvec_sm['i'] = smooth_data(rvec['i'], tvec['i'], win_len=sm_len) 
...
plt.figure()
plt.plot(tvec['e'], rvec['e'], label='E')
plt.plot(tvec_sm['e'], rvec_sm['e'], label='E')
plt.ylim([0,60])
